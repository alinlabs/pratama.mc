...
export function TamuList({ tamu = [] }: { tamu: TamuType[] }) {
  if (!tamu || tamu.length === 0) return null;
  return (
...
export default function TamuView({ event, onEdit }: { event: any; onEdit?: () => void }) {
  const [isExpanded, setIsExpanded] = useState(true);
  const tamu = event.tamu || [];
              ...
              <div className="flex flex-col gap-4 pt-2">
                ...
                {!tamu.length && (
                  <div className="py-12 text-center bg-stone-50/50 rounded-2xl border border-dashed border-stone-200">
                    ...
                    <p className="text-stone-500 text-sm">Tidak ada daftar tamu ditemukan.</p>
                  </div>
                )}
              </div>
            </motion.div>
