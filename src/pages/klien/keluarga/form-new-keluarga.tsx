import React, { useState } from 'react';
import { saveKlienKeluarga } from '../../../lib/api';
import { formatPhoneInput, formatSocialInput } from '../../../lib/inputFormatters';
import { Check, Users, Asterisk } from 'lucide-react';
import ComboBox from '../../../components/input-combobox';

export default function ShareableKeluargaForm({ event, onSaved, hideBanner }: { event: any, onSaved: () => void, hideBanner?: boolean }) {
  const [saving, setSaving] = useState(false);
  const [successMsg, setSuccessMsg] = useState('');
  
  const [formData, setFormData] = useState({
    peran: '',
    nama: '',
    nama_panggilan: '',
    whatsapp: '',
    instagram: '',
    isCustom: true
  });

  const PERAN_OPTIONS = [
    'Ayah Kandung Pria',
    'Ibu Kandung Pria',
    'Ayah Kandung Wanita',
    'Ibu Kandung Wanita',
    'Kakak Pengantin',
    'Adik Pengantin',
    'Paman / Bibi',
    'Eyang / Kakek / Nenek',
    'Keluarga Lainnya'
  ];

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.nama.trim() || !formData.nama_panggilan.trim() || !formData.whatsapp.trim() || !formData.peran.trim()) {
      alert("Nama Lengkap, Nama Panggilan, WhatsApp, dan Hubungan Keluarga wajib diisi.");
      return;
    }
    
    setSaving(true);

    try {
      const currentPanitia = event.panitia_keluarga || [];
      const currentTamu = event.tamu || [];
      const currentPengisiAcara = event.pengisi_acara || [];
      const currentPendamping = event.pendamping || [];
      
      const keluargaIntiList = event.keluarga_inti || [];
      const newKeluargaIntiList = [...keluargaIntiList, formData];

      await saveKlienKeluarga(event.id_klien, { 
        keluarga_inti: newKeluargaIntiList,
        panitia_keluarga: currentPanitia,
        tamu: currentTamu,
        pengisi_acara: currentPengisiAcara,
        pendamping: currentPendamping
      });

      setSuccessMsg(`Data keluarga (${formData.peran}) berhasil disimpan. Terima kasih!`);
      setTimeout(() => {
        onSaved();
      }, 2000);

    } catch (err) {
      console.error(err);
      alert('Terjadi kesalahan saat menyimpan data.');
    } finally {
      setSaving(false);
    }
  };

  if (successMsg) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
        <div className="w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center mb-6">
          <Check className="w-8 h-8 text-emerald-600" />
        </div>
        <h2 className="text-xl font-bold text-stone-800 mb-2 text-center">Berhasil!</h2>
        <p className="text-stone-600 text-center">{successMsg}</p>
      </div>
    );
  }

  return (
    <div className="w-full pb-20 md:pb-0">
      {/* Full Bleed Banner */}
      {!hideBanner && (
        <div className="w-full relative aspect-[4/3] md:aspect-[3/1] overflow-hidden bg-stone-900 flex mb-8">
          <picture className="w-full h-full flex-shrink-0">
            <source media="(min-width: 768px)" srcSet="/gambar/banner/desktop1.webp" />
            <img
              src="/gambar/banner/mobile1.webp"
              alt="Hero Banner"
              className="w-full h-full object-cover"
            />
          </picture>
        </div>
      )}

      <div className="w-full max-w-3xl mx-auto space-y-8 px-4 md:px-0">
        <div className="mb-4 text-center md:text-left">
          <h1 className="text-xl sm:text-3xl font-extrabold text-stone-900 mb-2">Area Keluarga Utama</h1>
          <p className="text-sm text-stone-500 sm:text-lg px-2 md:px-0">
            Terima kasih telah melengkapi data keluarga inti. Masukkan data hubungan keluarga Anda dengan pengantin ({event.username}).
          </p>
        </div>

        <form id="keluarga-form" onSubmit={handleSave} className="space-y-6" onKeyDown={(e) => {
            if (e.key === 'Enter' && e.target.tagName !== 'TEXTAREA') {
              e.preventDefault();
              const form = e.target.closest('form');
              if (!form) return;
              const focusableElements = Array.from(form.querySelectorAll('input:not([type="hidden"]), select, textarea, button[type="submit"]'));
              const index = focusableElements.indexOf(e.target);
              if (index > -1 && focusableElements[index + 1]) {
                focusableElements[index + 1].focus();
              }
            }
          }}>
          <h2 className="text-lg font-bold flex items-center gap-2 mb-6 text-stone-800">
             <Users className="w-5 h-5 text-[#DCAF43]" /> Formulir Hubungan Keluarga
          </h2>
          
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4 sm:gap-6">
              <div className="col-span-2">
                <label className="block text-sm font-semibold text-stone-700 mb-2 flex items-center gap-1 flex justify-between items-center"><span>
                  Hubungan Keluarga <Asterisk className="w-3 h-3 text-red-500" />
                </span><span className="text-[10px] text-red-500/50 italic font-normal">Wajib Diisi</span></label>
                <ComboBox
                  required
                  options={PERAN_OPTIONS.map(p => ({ label: p, value: p }))}
                  value={formData.peran}
                  onChange={(val) => setFormData({ ...formData, peran: val })}
                  placeholder="Ketik atau pilih peran..."
                />
              </div>

              <div>
                <label className="block text-[10px] sm:text-sm font-semibold text-stone-700 mb-2 flex items-center gap-1 truncate flex justify-between items-center"><span>
                  Nama Lengkap <Asterisk className="w-3 h-3 text-red-500" />
                </span><span className="text-[10px] text-red-500/50 italic font-normal">Wajib Diisi</span></label>
                <input
                  type="text"
                  required
                  placeholder="Contoh: Drs. H. Rohman"
                  value={formData.nama}
                  onChange={(e) => setFormData({ ...formData, nama: e.target.value })}
                  className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl focus:ring-0 focus:ring-0 focus:border-[#DCAF43] focus:outline-none focus:ring-0 text-stone-800 text-sm transition-all"
                />
              </div>

              <div>
                <label className="block text-[10px] sm:text-sm font-semibold text-stone-700 mb-2 truncate flex justify-between items-center"><span>
                  Nama Panggilan <Asterisk className="w-3 h-3 text-red-500" />
                </span><span className="text-[10px] text-red-500/50 italic font-normal">Wajib Diisi</span></label>
                <input
                  type="text"
                  required
                  placeholder="Contoh: Pak Rohman"
                  value={formData.nama_panggilan}
                  onChange={(e) => setFormData({ ...formData, nama_panggilan: e.target.value })}
                  className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl focus:ring-0 focus:ring-0 focus:border-[#DCAF43] focus:outline-none focus:ring-0 text-stone-800 text-sm transition-all"
                />
              </div>

              <div className="col-span-2 md:col-span-1">
                <label className="block text-sm font-semibold text-stone-700 mb-2 flex justify-between items-center"><span>WhatsApp / Telepon <Asterisk className="w-3 h-3 text-red-500" /></span><span className="text-[10px] text-red-500/50 italic font-normal">Wajib Diisi</span></label>
                <input
                  type="text" inputMode="numeric"
                  required
                  placeholder="Contoh: 628123456780"
                  value={formData.whatsapp}
                  onChange={(e) => setFormData({ ...formData, whatsapp: formatPhoneInput(e.target.value) })}
                  className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl focus:ring-0 focus:ring-0 focus:border-[#DCAF43] focus:outline-none focus:ring-0 text-stone-800 text-sm transition-all"
                />
              </div>

              <div className="col-span-2 md:col-span-1">
                <label className="block text-sm font-semibold text-stone-700 mb-2">
                  Instagram Username (Opsional)
                </label>
                <input
                  type="text"
                  placeholder="Contoh: @rohman_int"
                  value={formData.instagram}
                  onChange={(e) => setFormData({ ...formData, instagram: formatSocialInput(e.target.value) })}
                  className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl focus:ring-0 focus:ring-0 focus:border-[#DCAF43] focus:outline-none focus:ring-0 text-stone-800 text-sm transition-all"
                />
              </div>
            </div>

            <div className="flex justify-end pt-4">
              <button
                type="submit"
                disabled={saving}
                className="w-full sm:w-auto px-8 py-3 bg-stone-900 text-white font-bold rounded-xl hover:bg-stone-800 transition-all shadow-md active:scale-95 disabled:opacity-50"
              >
                {saving ? 'Menyimpan...' : 'Simpan Data'}
              </button>
            </div>
          </div>
        </form>
      </div>
    </div>
  );
}
